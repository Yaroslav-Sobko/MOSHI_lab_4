...
def ant_colony_optimization(distance_matrix, num_ants, alpha, beta, evaporation_rate, q0, pheromone_initial,num_iterations):
    ...
    for iter in range(num_iterations):
        for i in range(num_ants):
            ...
            if random.random() < q0:
                # за кількістю ферментувибирається місто
                ...
                # вибір локації залежно від рандому відстані такількості ферменту
        ...
        for start in range(n):
            ...
            # оновлення выдстані до початкової точки
            distance += distance_matrix[path[-1]][start]

            if distance < shortest_distance:
                shortest_distance = distance
                shortest_path = path + [start]
        print(f"{iter+1}: shortest path length = {shortest_distance}, shortest path = {shortest_path}")
...
def plot_map(best_path, cities_amount):
    ...
    ax.plot(best_x + [best_x[0]], best_y + [best_y[0]], 'green', linewidth=1, alpha=0.7)
    ax.scatter(x, y, zorder=10, s=5, color='purple')
    plt.show()
...
def main():
    ...
    num_iterations = 50
    shortest_path, shortest_distance=ant_colony_optimization(distances,num_ants, alpha, beta, rho, q0, pheromone_initial, num_iterations)
